...
def plotRoc(fpr, tpr, auc, labels, linestyle, text=True, legend=True):
...
def makeRoc(y, predict_test, labels, linestyle='-', legend=True, text=True):
    if 'j_index' in labels:
        labels.remove('j_index')

    fpr, tpr, auc1 = rocData(y, predict_test, labels)
    plotRoc(fpr, tpr, auc1, labels, linestyle, legend=legend, text=text)
    return predict_test
